...
export const Card: React.FC<CardProps> = ({ card, isFlipped = false, onClick, className, delay = 0 }) => {
  ...
  return (
    <div 
      className={cn("relative w-48 h-72 sm:w-64 sm:h-96 cursor-pointer card-preserve-3d", className)} 
      onClick={handleClick}
    >
      <motion.div
      ...
      >
        {/* Card Back */}
        <div className="absolute inset-0 w-full h-full card-backface-hidden rounded-xl border-2 border-gold-500 shadow-[0_0_15px_rgba(212,175,55,0.3)] bg-mystic-800 overflow-hidden flex items-center justify-center">
          <div className="w-[80%] h-[90%] border border-gold-500/50 flex flex-col items-center justify-center relative">
            {/* Simple Rose Cross / Hexagram placeholder for card back */}
        ...
        {/* Card Front */}
        <div className="absolute inset-0 w-full h-full card-backface-hidden rotate-y-180 rounded-xl border-2 border-gold-400 shadow-[0_0_20px_rgba(212,175,55,0.5)] bg-mystic-900 overflow-hidden flex flex-col">
          {card ? (
            <>
